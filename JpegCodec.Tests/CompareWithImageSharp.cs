...
public class CompareWithImageSharp
{
    ...
    public void CompareBackhoe_PixelByPixel()
    {
        ...
        using Image<L8> isImage = Image.Load<L8>(path); // L8 = grayscale
        ...
        for (var y = 0; y < ourImage.Height && mismatches < 100; y++)
        {
            for (var x = 0; x < ourImage.Width && mismatches < 100; x++)
            {
                (byte ourR, _, _) = ourImage.GetPixel(x, y);
            ...
            for (var mcuX = 0; mcuX < 2; mcuX++)
            {
                ...
                for (var blockY = 0; blockY < 2; blockY++)
                {
                    for (var blockX = 0; blockX < 2; blockX++)
                    {
                        ...
                        {
                            (byte ourVal, _, _) = ourImage.GetPixel(pixelX, pixelY);
    ...
    public void FindOurPixelInImageSharp()
    {
        ...
        (byte ourVal, _, _) = ourImage.GetPixel(0, 0);
        ...
        var found = 0;
        for (var y = 0; y < isImage.Height && found < 10; y++)
        {
            for (var x = 0; x < isImage.Width && found < 10; x++)
            {
